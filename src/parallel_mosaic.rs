...
impl MosaicBuilder for ParallelMosaic {
    fn sum_tile_channels(&self, mosaic_factory: &MosaicFactory) -> Vec<u32> {
        ...
        tile_sum.par_iter_mut().enumerate().for_each(|(index, tile_sum_channel)| {
    ...
    fn calc_tile_average(&self, mosaic_factory: &MosaicFactory, tile_sum: &[u32]) -> (Vec<u8>, Vec<u8>) {
        ...
        let calc_tile_avg = || {
            tile_average.par_iter_mut().enumerate().for_each(|(index, tile_channel)| {
                *tile_channel = (tile_sum[index] / mosaic_factory.tile_pixels) as u8;
            });
        };

        let calc_global_avg = || {
            global_sum.par_iter_mut().enumerate().for_each(|(index, global_sum_channel)| {
    ...
    fn create_mosaic(&self, mosaic_factory: &MosaicFactory, tile_average: &[u8]) -> Vec<u8> {
        ...
        mosaic.par_chunks_mut(
            mosaic_factory.tile_side_length as usize* mosaic_factory.image_data.channels as usize
        ).enumerate().for_each(|(index, chunk)| {
            let tile_x = index % mosaic_factory.tiles_x as usize;
            let tile_y = index / (mosaic_factory.tiles_x * mosaic_factory.tile_side_length) as usize;
            let tile = (tile_y * mosaic_factory.tiles_x as usize + tile_x)
                * mosaic_factory.image_data.channels as usize;
            let pixel_slice = &tile_average[tile..tile + mosaic_factory.image_data.channels as usize];
            chunk.chunks_mut(mosaic_factory.image_data.channels as usize).for_each(|pixel_chunk| {
